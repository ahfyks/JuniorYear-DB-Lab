...
/** 2. List complete Marina table (all rows and all columns) **/
SELECT * FROM Marina
...
/** 7. List the (Boat) slip number for every boat with the Boat type Dolphin 28 that is located in marina 1. **/SELECT MarinaNum, SlipNum FROM MarinaSlip WHERE BoatType = 'Dolphin 28' AND MarinaNum = '1'/** 8. List the boat name for each boat located in a (Boat) slip whose length is between 25 and 30 feet. **/
SELECT BoatName FROM MarinaSlip WHERE Length >= 25 AND Length < 30
/** 9. List the (Boat) slip number for every (Boat) slip in marina 1 whose annual rental fee is less than $3,000. **/
SELECT SlipNum FROM MarinaSlip WHERE RentalFee < 3000  AND MarinaNum = '1'
/** 10. Labor is billed at the rate of $60 per hour. List the slip ID,category number, estimated hours, and estimated labor cost for every service request. To obtain the estimated labor cost, multiply the estimated hours by 60. Use the column name “EstimatedCost” for the estimated labor cost. **/SELECT SlipID, CategoryNum, EstHours,EstHours * 60 AS EstLaborCost  FROM ServiceRequest/** HOMEWORK **/SELECT * FROM MarinaSlip/** 11. List the marina number and (Boat) slip number for all slips containing a boat with the type Sprite 4,000, Sprite 3,000  or Ray 4,025. **/
SELECT MarinaNum, SlipNum FROM MarinaSlip WHERE BoatType = 'Sprite 4000' OR BoatType = 'Sprite 3000' OR BoatType = 'Ray 4025' 
/** 12. List the marina number, (Boat) slip number and boat name for all boats. Sort the results by boat name within the marina number. **/
SELECT MarinaNum, SlipNum, BoatName FROM MarinaSlip ORDER BY BoatName, MarinaNum ASC
/** 13. How many Dolphin 25 boats are stored at both marinas? **/
SELECT COUNT( BoatName) FROM MarinaSlip WHERE BoatType = 'Dolphin 25'
/** 14. Calculate the total rental fees Alexamara receives each year based on the length of the slip. **/
SELECT Length, SUM(RentalFee) FROM MarinaSlip GROUP BY Length
...
SELECT MarinaNum, SlipNum, BoatName,MarinaSlip.OwnerNum ,FirstName, LastName FROM MarinaSlip INNER JOIN Owner ON MarinaSlip.OwnerNum = Owner.OwnerNum
...
SELECT SlipID, Description, Status FROM ServiceRequest WHERE CategoryNum = '1'
/** 17. For every service request for routine engine maintenance, list the (Boat) slip ID, marina number, slip number, estimated hours, spent hours, owner number and owner’s last name. **/SELECT MS.SlipID, MS.SlipNum, SR.EstHours, SR.SpentHours, MS.OwnerNum, O.LastName, SR.CategoryNum 
FROM MarinaSlip MS JOIN ServiceRequest SR ON MS.SlipID = SR.SlipID JOIN Owner O ON O.OwnerNum = MS.OwnerNum  
WHERE SR.CategoryNum = '1'
